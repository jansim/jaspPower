...
test1VarClass <- R6::R6Class(
  ...
  private = list(
    ...
    .populatePowerCurveESText = function(r, lst) {
      ...
      d50 <- pwr.var.test(n = n, sig.level = alpha, power = .5, alternative = alt)$rho
    ...
    .preparePowerContour = function(r, lst) {
      ...
      z.pwr <- sapply(dd, function(delta) {
        pwr.var.test(n = nn, d = delta, sig.level = alpha, alternative = alt)$power
      })
    ...
    .preparePowerCurveES = function(r, lst) {
      ...
      y <- pwr.var.test(n = n, rho = dd, sig.level = alpha, alternative = alt)$power
    ...
    .preparePowerCurveN = function(r, lst) {
      ...
      y <- pwr.var.test(n = nn, rho = d, sig.level = alpha, alternative = alt)$power
