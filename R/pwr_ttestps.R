...
.populatePowerTabTtestPS <- function(jaspResults, options, r, lst) {
  ...
  if (calc == "sampleSize") {
    if (round(pwr::pwr.t.test(n = n, d = d, sig.level = alpha, alternative = alt, type = "paired")$power, 3) == 1) {
      ...
      table$addFootnote(gettextf(
        ...
        round(pwr::pwr.t.test(n = n, d = d, sig.level = alpha, alternative = alt, type = "paired")$power, 3)
      ))
    }
  }
}
