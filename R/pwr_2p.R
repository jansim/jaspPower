...
.runTest2P = function(jaspResults, options) {
  ...
  if (options$text) {
    .initPowerESTabTest2P(jaspResults, options, results, stats)
  }
  ...
  if (options$powerContour) {
    .preparePowerContourTest2P(jaspResults, options, results, stats)
    if (options$text) {
      .populateContourTextTest2P(jaspResults, options, results, stats)
    }
  }
  if (options$powerByEffectSize) {
    .preparePowerCurveESTest2P(jaspResults, options, results, stats)
    if (options$text) {
      .populatePowerCurveESTextTest2P(jaspResults, options, results, stats)
    }
  }
  if (options$powerBySampleSize) {
    .preparePowerCurveNTest2P(jaspResults, options, results, stats)
    if (options$text) {
      .populatePowerCurveNTextTest2P(jaspResults, options, results, stats)
    }
  }
  if (options$powerDemonstration) {
    .preparePowerDistTest2P(jaspResults, options, results, stats)
    if (options$text) {
      .populateDistTextTest2P(jaspResults, options, results, stats)
    }
  }
  if (options$saveDataset && options$savePath != "") {
    .generateDatasetTest2P(jaspResults, options, results, stats)
  }
}
